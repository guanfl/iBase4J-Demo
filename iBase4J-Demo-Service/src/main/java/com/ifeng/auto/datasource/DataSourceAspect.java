...
*/
package com.ifeng.auto.datasource;
...
public class DataSourceAspect {
    ...
    private static final String[] defaultSlaveMethodStart = new String[]{ "select","query", "find", "get" };
    ...
    public void before(JoinPoint point) {
        ...
        if (isSlave) {
            DynamicDataSourceHolder.markSlave();// 标记为读库
        } else {
            DynamicDataSourceHolder.markMaster();// 标记为写库
        }
    }
    ...
    public String[] getSlaveMethodStart() {
        if(this.slaveMethodStart == null){
            return defaultSlaveMethodStart;// 没有指定，使用默认
        }
